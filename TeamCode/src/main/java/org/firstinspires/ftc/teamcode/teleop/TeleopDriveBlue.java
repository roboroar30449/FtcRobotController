...
public class TeleopDriveBlue extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.x && !buttonPressed) {
                ...
                sleep(6000);
                while (opModeIsActive() && BlockDetection_Blue.deltaX == 0) {
                    blockDetection_Blue.allVisionTelemetry();
                    mechController.handleMechState(MechState.SUB_POSITION);
                    idle();
                }
                sleep(6000);
                robot.webcamClaw.stopStreaming();
                robot.webcamClaw.closeCameraDevice();
                mechController.handleMechState(MechState.DETECTED_POSITION_BLUE);
    ...
    public void detect_Blue() {
        blockDetection_Blue = new BlockDetection_Blue(robot, telemetry);
        robot.webcamClaw.setPipeline(blockDetection_Blue.pipeline);
        cameraOpened = false; // Flag to track if camera is ready

        robot.webcamClaw.openCameraDeviceAsync(new OpenCvCamera.AsyncCameraOpenListener() {
            @Override
            public void onOpened() {
                robot.webcamClaw.startStreaming(640, 480, OpenCvCameraRotation.UPRIGHT);
                cameraOpened = true;
            }
